...
if (  !$args{'term_name'} && !$args{'date_range'}
    || $args{'term_name'} && $args{'date_range'} )
    {
    ...
    if ( $term_id eq 'no match' )
        ...
        @start_date{ 'yyyy', 'mm', 'dd' } =
            ( $start_date{'yyyy-mm-dd'} =~ m/(\d\d\d\d)-(\d\d)-(\d\d)/ );
        @end_date{ 'yyyy', 'mm', 'dd' } =
            ( $end_date{'yyyy-mm-dd'} =~ m/(\d\d\d\d)-(\d\d)-(\d\d)/ );
        }
...
# must be to a list.
@{ $args{'start_time'} }{ 'hh', 'mm' } =
    $args{'start_time'}{'actual'} =~ /(\d\d)(\d\d)/;
@{ $args{'end_time'} }{ 'hh', 'mm' } =
    $args{'end_time'}{'actual'} =~ /(\d\d)(\d\d)/;
...
if ( !$args{'start_time'}{'actual'} || !$args{'end_time'}{'actual'} )
    ...
    my %result;
    @result{ 'dd', 'hh', 'mm', 'ss' } =
        Date::Calc::Delta_DHMS( 2011, 5, 23,
        @{ $args{'start_time'} }{ 'hh', 'mm' },
        0, 2011, 5, 23, $hours{'weekday_start'}, 0, 0 );
    if ( $result{'hh'} > 0 )
        {
    ...
    @result{ 'dd', 'hh', 'mm', 'ss' } =
        Date::Calc::Delta_DHMS( 2011, 5, 23,
        @{ $args{'start_time'} }{ 'hh', 'mm' },
        0, 2011, 5, 23, $hours{'weekday_end'}, 0, 0 );
    if ( $result{'hh'} < 1 )
        {
    ...
    @result{ 'dd', 'hh', 'mm', 'ss' } =
        Date::Calc::Delta_DHMS( 2011, 5, 23,
        @{ $args{'start_time'} }{ 'hh', 'mm' },
        0, 2011, 5, 23, $hours{'weekend_start'}, 0, 0 );
    if ( $result{'hh'} > 0 )
        {
        print "Specified start time is too early.\n";
        $die_early = 1;
    ...
    @result{ 'dd', 'hh', 'mm', 'ss' } =
        Date::Calc::Delta_DHMS( 2011, 5, 23,
        @{ $args{'start_time'} }{ 'hh', 'mm' },
        0, 2011, 5, 23, $hours{'weekend_end'}, 0, 0 );
    if ( $result{'hh'} < 1 )
        {
        print "Specified start time is too late.\n";
        $die_early = 1;
    ...
    if ( Date::Calc::Day_of_Week( @current_date{ 'yyyy', 'mm', 'dd' } ) ==
        ...
        while ( $current_hour < $args{'end_time'}{'hh'} )
            ...
            if ( $shift_id != 0 )
                ...
                if ( check_shift( $args{'dog_id'}, $shift_id, $args{'desk_id'} )
                    )
                    {
                    ...
                    new_assignment( $args{'dog_id'}, $shift_id,
                        $args{'desk_id'} );
                    }
                }
            else
