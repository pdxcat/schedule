...
sub ho_add
    ...
    foreach my $id ( keys( %{$ho_ref} ) )
        ...
        if (
            (
                   $ho_ref->{$id}->{'ns_holiday_name'} eq $args_ref->{'name'}
                && $ho_ref->{$id}->{'ns_holiday_date'} eq $args_ref->{'date'}
            )
            || $ho_ref->{$id}->{'ns_holiday_date'} eq $args_ref->{'date'}
            )
            {
...
sub ho_del_name
    ...
    my $ho_ref   = ho_get();
    my $sth_del_ho =
        $dbh->prepare('DELETE FROM ns_holiday WHERE ns_holiday_name = ?');
...
sub ho_del_date
    ...
    my $ho_ref   = ho_get();
    my $sth_del_ho =
        $dbh->prepare('DELETE FROM ns_holiday WHERE ns_holiday_date = ?');
