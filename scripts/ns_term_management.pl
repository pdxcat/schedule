...
sub term_add
    ...
    foreach my $id ( keys( %{$term_ref} ) )
        {
        if (   $term_ref->{$id}->{'ns_term_name'} eq $args_ref->{'name'}
            || $term_ref->{$id}->{'ns_term_startdate'} eq
            $args_ref->{'start_date'}
            || $term_ref->{$id}->{'ns_term_enddate'} eq $args_ref->{'end_date'}
            )
            {
...
sub term_del
    ...
    my $term_ref = term_get();
    my $sth_del_term =
        $dbh->prepare('DELETE FROM ns_term WHERE ns_term_name = ?');
