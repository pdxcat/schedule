...
} elsif ($datearg eq "" && $rangearg =~ /^(\d{4})-(\d{2})-(\d{2}) (\d{4})-(\d{2})-(\d{2})$/) {
	# If we got a range argument only...
	my @rangecurrent = ($1,$2,$3);
	my @rangeend = ($4,$5,$6);

	while (Date::Calc::Delta_Days(@rangecurrent[0..2],@rangeend[0..2]) >= 0) {
		$date = strftime("%a %b %e",0,0,0,$rangecurrent[2],$rangecurrent[1]-1,$rangecurrent[0]-1900);
		$shortdate = strftime("%b%d", 0,0,0,$rangecurrent[2],$rangecurrent[1]-1,$rangecurrent[0]-1900);
